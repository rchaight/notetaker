...
- [x] ADP nudge (no public detection API — Settings guidance); per-note AI-tier disclosure (every AI block stamps its provider; Settings states the policy). WKWebView sanitization N/A — the app has no web preview surface (HTML imports convert to markdown).
- [x] Hardened Runtime verified on the installed build (codesign flags 0x10000(runtime), no exception entitlements). App Sandbox intentionally absent per the M5 decision (Developer ID distribution + local engine Process spawning); re-audit at M10 notarization.
...
- [x] "Today's Tasks" widget (small/medium, macOS desktop + iOS home screen): app publishes a today-snapshot to the app-group container on every reindex, WidgetCenter reload keeps it fresh. *(Interactive inline check-off + Quick Note widget deferred — needs the intent to run sandboxed in the widget process; logged.)*
...
- [~] (Optional) Apple Watch complication — SKIPPED for v1 (no watch target; revisit post-release if requested). M9 COMPLETE.
...
### M9.5 — Shakedown + Settings buildout (user-directed, before M10)
...
- [x] Settings menu buildout: six panes (General/Editor/To-Do/Security/Vault/AI & Import + Debug), every persistent knob surfaced, shortcut reference, folder-vault management with Remove.
