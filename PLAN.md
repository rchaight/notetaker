...
- [x] Router: pick tier by input type + platform + connectivity; **iCloud import-inbox** fallback (iOS drops source → Mac converts → syncs back). *(Router shipped: native-first, docling-serve for office formats, descriptive errors. Import-inbox relay deferred until the Mac-local Docling tier exists.)*
